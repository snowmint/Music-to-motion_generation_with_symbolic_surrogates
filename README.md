...
6. The generated MP4 video file will appear in the file panel on the left-hand side of the Colab page, as shown in the diagram below. Double-click the video file to download and view the results. **After downloading the resulting video each time, remember to delete the files on Colab. Videos generated from the same music will have the same file names, causing new output videos to be unable to overwrite previous files.**: <br/>
