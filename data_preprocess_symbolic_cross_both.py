...
for this_performer in performaer_list:
    ...
    for item_path in audio_path_list:
        ...
        align_file_path = "./UoE_violin_midi/[Final_Ensure]ev/"+compoer_id_new + \
            "/"+performer_id_new+"/"+try_count+"/annotations/"+align_filename
        cadence_file_path = "./UoE_violin_midi/[Final_Ensure]ev/"+compoer_id_new + \
            "/"+performer_id_new+"/"+try_count+"/annotations/"+cadence_filename
        expression_file_path = "./UoE_violin_midi/[Final_Ensure]ev/"+compoer_id_new + \
            "/"+performer_id_new+"/"+try_count+"/annotations/"+expression_filename
        phrase_file_path = "./UoE_violin_midi/[Final_Ensure]ev/"+compoer_id_new + \
            "/"+performer_id_new+"/"+try_count+"/annotations/"+phrase_filename
        beat_file_path = "./UoE_violin_midi/[Final_Ensure]ev/"+compoer_id_new + \
            "/"+performer_id_new+"/"+try_count+"/annotations/"+beat_filename
        downbeat_file_path = "./UoE_violin_midi/[Final_Ensure]ev/"+compoer_id_new + \
            "/"+performer_id_new+"/"+try_count+"/annotations/"+downbeat_filename
        harmony_file_path = "./UoE_violin_midi/[Final_Ensure]ev/"+compoer_id_new + \
            "/"+performer_id_new+"/"+try_count+"/annotations/"+harmony_filename
        ...
        audio_time_df = pd.DataFrame(
            np.arange(0, (audio_seq_len * (1/change_fps)), (1/change_fps))[:, None])
        ...
        downbeat_df['downbeat_align'] = downbeat_df['downbeat'].map(
            downbeat_map)
        ...
        dynamic_dict = {'ppp': 0.125, 'pp': 0.25, 'p': 0.375,
                        'mp': 0.5, 'mf': 0.625, 'f': 0.75, 'ff': 0.875, 'fff': 1}
        ...
        for index, row in expression_df.iterrows():
            ...
            if row['expression'] in dynamic_dict:
                ...
                filter_expression = (alignment_expression_df['score_position'] >= row['onset']) & (
                    alignment_expression_df['score_position'] < row['offset'])
                # Update the 'dynamic' column for the filtered rows
                alignment_expression_df.loc[filter_expression,
                                            'dynamic'] = dynamic_value

        alignment_expression_df.loc[:, ['dynamic']] = alignment_expression_df.loc[:, [
            'dynamic']].replace(0.0, np.NaN)
        alignment_expression_df.loc[:, [
            'dynamic']] = alignment_expression_df.loc[:, ['dynamic']].ffill()
        alignment_expression_df.loc[:, [
            'dynamic']] = alignment_expression_df.loc[:, ['dynamic']].bfill()
        ...
        # alignment_expression_df['staccato'].copy()
        audio_time_df['staccato'] = 0
        ...
        # alignment_expression_df['dynamic'].copy()
        audio_time_df['dynamic'] = 0
        audio_time_df['beat'] = 0
        audio_time_df['downbeat'] = 0
        audio_time_df['harmony_degree'] = 0
        ...
        annotation_extend = audio_time_df[['cadence', 'phrase', 'legato', 'staccato', 'accent',
                                           'rit', 'accel', 'cresc', 'dim', 'dynamic', 'beat', 'downbeat', 'harmony_degree']].to_numpy()
        ...
        save_out = np.concatenate(
            (piano_roll_dictionary[this_performer][name_code], mfcc_dictionary[this_performer][name_code]), axis=1)
        print(save_out.shape)
        # raise()
        # Save as pickle
        if not os.path.exists('./preprocessed_data_save_cross_both/all/'):
            os.makedirs('./preprocessed_data_save_cross_both/all/')

        combine_data_output = open('./preprocessed_data_save_cross_both/all/' +
                                   name_code + '_all_data.pkl', 'wb')
        pickle.dump(save_out, combine_data_output)
        combine_data_output.close()

        ####
        # append annotation to target data
        Row_list = target_dict[name_code]

        Row_list_extend = np.append(
            np.array(Row_list), annotation_extend_dict[name_code], axis=1)
        print("Row_list_extend.shape:", Row_list_extend.shape)
        # np.array(Row_list)
        keypoint_dictionary[this_performer][name_code] = Row_list_extend

        if not os.path.exists('./preprocessed_data_save_cross_both/motion/'):
            os.makedirs('./preprocessed_data_save_cross_both/motion/')

        motion_data_output = open('./preprocessed_data_save_cross_both/motion/' +
                                  name_code + '_motion_data.pkl', 'wb')
...
for this_performer in performaer_list:
    for piece_name in piece_list:
        ...
        each_audio_motion_len_compare[this_performer + '_' + piece_name]["all"] = len(
